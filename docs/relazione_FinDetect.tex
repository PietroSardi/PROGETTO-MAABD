...
un'osservazione semplice: il modello relazionale \`e ottimo quando i dati sono
``piatti e omogenei'', ma diventa scomodo quando l'informazione interessante
sta nelle \emph{relazioni fra le entit\`a}. Trovare un ciclo di
...
nozione \`e spiegata in modo intuitivo e collegata a come \`e stata usata nel
progetto.
...
(le relazioni), entrambi dotati di propriet\`a. Ha senso sceglierlo dove la
natura dei dati \`e gi\`a una rete di relazioni, come nel nostro caso. La forza
del modello \`e che abilita le \textbf{query di raggiungibilit\`a}, cio\`e
interrogazioni che chiedono se e come due nodi sono collegati da un cammino,
che nel relazionale sono molto costose da esprimere e da eseguire. \`E
esattamente il caso dei nostri cicli di transazioni (Q4) e della ricerca degli
snodi (Q6).
...
grado} (\emph{degree centrality}): il grado di un nodo \`e il numero di archi
\texttt{TRANSFER} che lo toccano, contando sia le transazioni entranti sia
quelle uscenti. Un conto con grado molto alto \`e un candidato hub: nello
scenario antifrode \`e un punto che merita attenzione. Q6 calcola questo grado
\emph{dentro una finestra temporale} e offre due criteri per decidere chi \`e
hub:
\begin{itemize}[nosep]
  \item \textbf{soglia assoluta} (parametro \texttt{minDegree}, il default):
  tiene i conti con grado maggiore o uguale alla soglia. \`E il criterio \emph{operativo},
...
\textbf{selettivit\`a} di una condizione: la frazione dei dati in ingresso che
supera il filtro. Un filtro molto selettivo lascia passare poche tuple, uno
poco selettivo costringe il motore a toccare gran parte dei dati.
Le nostre query di lookup sono molto selettive (filtrano su un singolo
...
stream processing). Va distinta la ``consistenza'' transazionale di un DBMS
centralizzato (la C di ACID: ogni transazione porta la base di dati da uno stato
valido a un altro) dalla consistenza di un sistema \emph{distribuito}, che
riguarda invece quanto le copie di un dato su nodi diversi sono allineate fra
loro. Per gli aspetti distribuiti ci appoggiamo alla documentazione ufficiale
dei sistemi usati, collegandoli a come il prototipo \`e effettivamente
configurato.
...
in un DBMS puramente centralizzato.
...
sequenza: prima Neo4j trova gli hub (i conti con grado alto), poi MongoDB
arricchisce ciascun hub con tipo di conto, data
...
temporale, calcolare il grado di ogni conto e tenere
...
dopo, restano nel buffer e le query a regime sono molto pi\`u veloci. \`E il
principio di \emph{localit\`a temporale} su cui si basa la gestione del buffer:
...
pi\`u dati la query \`e costretta a toccare, pi\`u tempo impiega. \`E il principio
di base dell'ottimizzazione delle interrogazioni.
...
dove rende di pi\`u. L'analisi dei tempi conferma quanto anticipato nella
Sezione~2: il costo
